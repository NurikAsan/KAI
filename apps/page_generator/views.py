...
@extend_schema(tags=['NavLinks'])
# class NavView(CacheMixin, generics.ListAPIView, generics.RetrieveAPIView):
class NavView(generics.ListAPIView, generics.RetrieveAPIView):
    # CACHE_KEY_PREFIX = "nav_links"
    queryset = NavLinks.objects.all()
    serializer_class = NavLinksSerializer


@extend_schema(tags=['TextContent'])
# class TextContentView(CacheMixin, generics.ListAPIView, generics.RetrieveAPIVie):
class TextContentView(generics.ListAPIView, generics.RetrieveAPIView):
    # CACHE_KEY_PREFIX = "text_content"
    queryset = TextContent.objects.all()
    serializer_class = TextContentSerializer


@extend_schema(tags=['Veterans'])
# class VeteranView(CacheMixin, generics.ListAPIView, generics.RetrieveAPIView):
class VeteranView(generics.ListAPIView, generics.RetrieveAPIView):
...
# class PartnersView(CacheMixin, generics.ListAPIView, generics.RetrieveAPIView):
class PartnersView(generics.ListAPIView, generics.RetrieveAPIView):
